...
class _CostFormDialogState extends ConsumerState<CostFormDialog> {
  ...
  Future<void> _save() async {
    ...
    if (isPaying && _evidencia == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('A evidência é obrigatória para confirmar o pagamento.'), backgroundColor: AppTokens.warning),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erro: $e'), backgroundColor: AppTokens.error));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: ConstrainedBox(
        constraints: EduFormStyles.dialogConstraints(context, maxWidth: isPaid && !isAdmin ? 500 : 440),
        child: Padding(
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text('Detalhes do custo', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
        ...
        const SizedBox(height: 24),
        SizedBox(width: double.infinity, child: OutlinedButton(onPressed: () => Navigator.pop(context), child: const Text('Fechar'))),
      ],
    );
  }
  ...
  Widget _buildForm(bool isPaying, bool isNew, bool isAdmin) {
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          Text(
            isPaying ? 'Liquidar custo' : (isCorrection ? 'Corrigir dados' : 'Adicionar custo'),
          ...
          EduFormStyles.dialogActions(
            ...
            confirmLabel: isPaying ? 'Confirmar' : (isCorrection ? 'Corrigir' : 'Adicionar'),
            isLoading: _isSaving,
          ),
        ],
      ),
  ...
  Widget _buildImageActionSmall(IconData icon, VoidCallback onTap) {
    return Container(
      decoration: BoxDecoration(color: AppTokens.primaryDark, borderRadius: BorderRadius.circular(4)),
      child: IconButton(
  ...
  Widget _buildField(String label, TextEditingController controller, IconData icon, {bool isNumber = false, bool capitalize = false, bool enabled = true}) {
    return TextFormField(
      ...
      decoration: EduFormStyles.inputDecoration(label, icon: icon, enabled: enabled),
    ...
    return InkWell(
        ...
        child: Container(
            ...
            child: Column(
                ...
                children: [
                  Icon(icon, color: AppTokens.primary, size: 18),
                  Text(label,
